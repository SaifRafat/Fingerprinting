...
class TaggedCSVDocument(object):
    """Iterator yielding tagged documents from CSV file (Quora format)."""
    ...
    def __iter__(self):
        ...
        # Process question1 column
        if 'question1' in self.df.columns:
            for text in self.df['question1'].dropna():
                words = simple_preprocess(str(text))
                if words:
                    yield g.doc2vec.TaggedDocument(words=words, tags=[str(doc_id)])
                    doc_id += 1
        
        # Process question2 column
        if 'question2' in self.df.columns:
            for text in self.df['question2'].dropna():
                words = simple_preprocess(str(text))
                if words:
                    yield g.doc2vec.TaggedDocument(words=words, tags=[str(doc_id)])
                    doc_id += 1
...
def batch_train_experiments():
    ...
    csv_file = r'E:\Research work\wenggang\paper\code_alexa\doc2vec_model\toy_data\quora\Quora_dataset.csv'
    
    # Create models folder
    models_folder = os.path.join(SCRIPT_DIR, 'quora_models')
    ...
    for train_epoch in epochs_list:
        saved_path = os.path.join(models_folder, f'quora_e{train_epoch}_v{vector_size}.bin')
